    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0: #每多少次输出在训练集和校验集上的效果
                ...
                # train_acc = torchmetrics.functional.accuracy(predit,true)
...
def evaluate(config, model, dev_iter, test=False, flag = 0):
    """

    :param config:
    :param model:
    :param dev_iter:
    :param flag:用来直接返回预测值
    :return:
    """
    model.eval()
    loss_total = 0
    ...
    with torch.no_grad():
        for texts, labels in dev_iter:
            ...
            if flag:
                return config.class_list[predict[1]]
...
def verify_predict(config, model, text):
    contents = []
    ...
    if pad_size:
        if len(token) < pad_size:
            ...
            seq_len = pad_size
    result = [(token_ids, 0, seq_len, mask)] * config.batch_size
    contents.extend(result)
    content_iter = utils.bulid_iterator(contents, config)
    model.load_state_dict(torch.load(config.save_path))
    result = evaluate(config, model, content_iter, flag=1)
    return result
