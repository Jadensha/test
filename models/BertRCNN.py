...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.cnn = nn.ModuleList(
            [nn.Conv2d(in_channels=1, out_channels=config.num_filters, kernel_size=(k, config.hidden_size)) for k in
             config.filter_sizes]
        ) #  新增卷积层
    ...
    def forward(self, x):
        ...
        out = out.unsqueeze(1)
        out = torch.cat([self.conv_and_pool(out, conv) for conv in self.cnn], 1)
